...
###  Core Features (Python Analysis)
- Reads experimental data from a CSV file (`xy_data.csv`).
- Fits a **non-linear parametric model** of the form:
  \[
  (t*\cos(\theta) - e^{-k|t|}\sin(\omega t)\sin(\theta) + x_0,\  
  y_0 + t*\sin(\theta) + e^{-k|t|}\sin(\omega t)\cos(\theta))
  \]
- Optimizes model parameters using **non-linear least squares**.
- Generates and visualizes fitted curve (`curve_result.png`).
- Displays estimated parameters and fitting accuracy (`results.png`).
